...
**No silent failure.** Every fatal precondition (missing `VJ_VERIFY_RUN_ID`, or an incomplete/misnamed R2 config -- the F17 class, creds passed as `R2_S3_*`/`AWS_*` instead of the `R2_*` names the store reads) emits a structured terminal `@event verify_fatal {stage, missing}` to stdout BEFORE exiting non-zero (the `missing` list names the absent env vars, so the harness diagnoses it without prose-parsing). A launch-side env mistake can never present as a 30-minute empty prefix with nothing to read. And a present-but-BAD R2 credential (a quote-wrapped / malformed value that passes the presence check but fails auth on write) is caught by an AUTHORITATIVE first channel write: the emitter proves the R2 channel is writable before any probe/render, and on failure emits `@event verify_fatal {stage: r2_auth}` rather than letting the best-effort per-event write swallow the auth error and silently empty the channel.
