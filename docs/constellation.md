...
> **You are here: `vivijure-backend` is the GPU render engine box.** It is the cloud GPU that
> rents time on RunPod, trains a character face model (LoRA), draws the keyframes, and turns them
> into video. The Studio in the center hands it the work; this repo does the heavy rendering.
