...
| backend-v1.0.11 | 1.0.11 | 9fe77f5 | 2026-07-25 (GHA) | **`av` (PyAV) 13.1.0 -> 18.0.0, a deliberate major bump (#313).** `finish.py` reads clips only through the imageio.v3 `pyav` plugin (`immeta`/`imiter`), never the `av` API directly; that exact path is smoke-tested identical across both versions. Render-verified 2026-07-25: the live pod verify gate (`runpod-verify`, run 30140653491) passed on RC image `1.0.11-rc1` built from this change -- keyframe, i2v and finish over the structured verify channel, teardown clean. Prod endpoints repin via the verify-gate promote leg on the release image. |
| backend-v1.0.10 | 1.0.10 | 0df8810 | 2026-07-23 (GHA) | **Cast-registry pretrained_loras (#332).** Accept `loras/cast-{id}/…` and `loras/lora-{slug}-…/…`; bundle_key tenancy unchanged. Src-only assemble; pin RunPod after verify+promote. |
| backend-v1.0.9 | 1.0.9 | 305ece9 | 2026-07-23 (GHA) | **K3 security closeout: escape `@event` JSON in `worker.py` (#327).** A value carrying a quote or a newline could break the structured progress channel the studio parses; now escaped. Plus `docs(security)`: the K3 medium/low false-positive disposition recorded rather than silently dismissed (#328). |
| backend-v1.0.8 | 1.0.8 | 0c199de | 2026-07-23 (GHA) | **KF3 closeout: scope harness `audio_key` reads (#326).** `renders/` audio beds must sit under `renders/<project>/`; flat `audio/<filename>` studio beds remain allowed (no nested `audio/` paths). Digest `sha256:9b05cf8c...`; RunPod render EP `t9wcvlxh8rc5la` (template `pdc3fsdqbc`, sls v207). |
| backend-v1.0.7 | 1.0.7 | 756f35a | 2026-07-23 (GHA) | **KF3 tenant R2 key binding (#324).** Harness scopes `bundle_key` / `keyframe_key` / `clip_key` / `pretrained_loras` to job project slug. Src-only assemble; no runtime rebuild. |
| backend-v1.0.6 | 1.0.6 | f19fbbc | 2026-07-22 (GHA) | **HF repo_id allowlist at cold start (#317).** Pin RunPod after verify+promote. |
| backend-v1.0.5 | 1.0.5 | 10fb2d7 | 2026-07-22 (GHA) | **docs(hub): RunPod Hub listing probe files (#308).** `.runpod/handler.py` and `.runpod/Dockerfile` become real files so the Hub probe can read them (root entries stay git symlinks for the package layout); `.runpod/README.md` gains the Hub R2 env blurb (#307), naming the trap that this backend reads `R2_ENDPOINT` while the satellites read `R2_ENDPOINT_URL`. Also the Jul 30 render `workersMax` floor and quota-30 note (#306). No runtime or weight change; `release.yml` assembles the image from the pinned runtime base. |
| backend-v1.0.4 | 1.0.4 | 49ffba5 | 2026-07-22 (GHA) | **Pillow 12.3.0** via runtime `runtime-1-bf16-t4` overlay (#295 / cf#178). Pin RunPod to `:1.0.4` after verify+promote. Do not pin `:1.0.3` (tagless; still t3). |
...
| backend-v1.0.0 | 1.0.0 | 1112328 | 2026-07-13 (GHA) | **First stable release of the GPU render backend (#254).** The clean-room RunPod serverless render engine (SDXL keyframes -> i2v -> assemble, plus LoRA training) that powers Vivijure Studio v1.0.0; the render contract (`docs/CONTRACT.md`) and the baked-image deploy path are stable. Ships on top of 0.4.9: promote smoke now uses a train-free bundle so a promote cannot be gated on a training path (#243); RunPod v2 `update-endpoint` 500 and account-quota gotchas banked in the deploy notes (#246, #247). Cut as part of the constellation-wide v1.0.0 milestone. |
|---|---|---|---|---|
